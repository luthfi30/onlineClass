...
@section('content')
 @if(count($errors) > 0)
    ...
    <a href="{{route('mentor.forum.show',$course->id)}}"  class="btn btn-primary mb-3">Back</a>
<form  action="{{ route('mentor.forum.update', $item->id) }}" method="POST" enctype="multipart/form-data" >
        @csrf
        @method('put')
