              ...
              @foreach ($forum->comment()->where('parent',0)->orderBy('created_at','desc')->get() as $komentar)
                ...
                <div class="media media-border-bottom py-3 align-items-center  px-5">
                    ...
                    <div class="media-body pr-3">
                        <a class="mt-0 mb-1 font-size-15 text-dark" href="#">{{$komentar->user->name}}</a>   <span class=" font-size-12 "><i class="mdi mdi-clock-outline"></i>  {{$komentar->created_at->diffForHumans()}}</span>
                        <p>{{$komentar->konten}}</p>
                ...
                @endforeach
                 ...
                 <form action="" method="POST">
                    ...
                    <input type="hidden" name="parent" value="{{$komentar->id}}">
                     <textarea name="konten" class="form-control" id="" rows="2"></textarea>
