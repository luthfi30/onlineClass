    ...
    <div class="row">
										...
										<div class="card-body " data-simplebar="" >
											...
											<div class="media media-border-bottom py-3 align-items-center justify-content-between border-bottom px-5">
												...
												<div class="media-body pr-3">
													<a class="mt-0 mb-1 font-size-15 text-dark" href="{{ route('mentor.forum.view', $item->id) }}">{{$item->user->name}} : {{$item->judul}} </a>
													<p><span class=" font-size-12 d-inline-block"><i class="mdi mdi-clock-outline"></i> {{$item->created_at->diffForHumans()}}</span></p>
												</div>

                                                @if($item->user_id === Auth::user()->id)

                                              <a href="{{route('mentor.forum.edit',$item->id)}}" class="btn btn-warning" ><i class="fa fa-edit"></i></a>
                                              
                                               <form action="{{ route('mentor.forum.destroy',$item->id) }}" method="POST">
                                                @csrf
                                                @method('DELETE')
                        ...
                        <div class="modal fade" id="modaltambahforum" tabindex="1" aria-labelledby="modaltambahforum" aria-hidden="true" >
                            <div class="modal-dialog" >
                                ...
                                <!--FORM TAMBAH BARANG-->
                                    <form action="{{ route('mentor.forum.store') }}" method="POST" enctype="multipart/form-data" >
                                            @csrf
                                        <div class="form-group">
                                            ...
                                            <div class="form-group">
                                              ...
                                              <input type="hidden" class="form-control" id="mycourse_id" placeholder="mycourse_id" value="{{$course->id}}" name="mycourse_id"  >
                                            </div>
